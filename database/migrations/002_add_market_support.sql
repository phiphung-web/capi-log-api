ALTER TABLE capi_event_logs
  ADD COLUMN IF NOT EXISTS market_key TEXT NOT NULL DEFAULT 'global';

ALTER TABLE capi_event_logs
  DROP CONSTRAINT IF EXISTS capi_event_logs_product_key_event_name_event_id_key;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'capi_event_logs_market_product_event_key'
  ) THEN
    ALTER TABLE capi_event_logs
      ADD CONSTRAINT capi_event_logs_market_product_event_key
